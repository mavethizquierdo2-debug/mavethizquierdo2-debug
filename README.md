<!DOCTYPE html>
<html lang="es">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
</head>
<body>
  <h1 align="center">👋 Hola, soy Maveth Izquierdo Hernández</h1>
    <h3 align="center">🎓 Estudiante de Ingeniería en Cenyca Universidad</h3>
    <hr>
 <!-- SOBRE MÍ -->
 <picture><img src="https://github.com/7oSkaaa/7oSkaaa/blob/main/Images/about_me.gif?raw=true" width="50px"></picture> Sobre mí  

<picture>
  <img align="right" src="https://github.com/7oSkaaa/7oSkaaa/blob/main/Images/Right_Side.gif?raw=true" width="250px">
</picture>

<br><br>

- 🎓 Soy estudiante de *Ingeniería en Sistemas Computacionales* en Cenyca Universidad.  
- 💼 Actualmente trabajo en *Balboa Water Group*.  
- 🎂 Tengo *23 años*.  
- 💻 Me apasiona la *programación*.  
- 🎨 Me gusta *dibujar y aprender cosas nuevas*.  
- 🌱 Siempre estoy abierto a *aprender, colaborar y crecer*.  

<br>

---

<!-- HABILIDADES -->
## <picture><img src="https://img.icons8.com/color/48/000000/rocket--v1.png" width="40px"></picture> Habilidades

- ⚡ *Aprendizaje rápido*  
- ✅ *Entrega y dedicación*  
- 🔥 *Motivación y ganas de trabajar*  
- 📂 *Organización*  
- 🤝 *Trabajo en equipo*  

<br>



  <p align="center">
    💻 Apasionado por la programación y el aprendizaje de nuevas tecnologías.  
    🚀 Actualmente aprendiendo <b>C#, Arduino, redes Cisco y LabVIEW</b>.
  </p>

  <hr>

  <h2>🔧 Tecnologías y herramientas</h2>
  <ul>
    <li><b>Lenguajes:</b> C# | HTML | C++ </li>
    <li><b>Hardware:</b> Arduino  </li>
    <li><b>Redes:</b> Cisco Packet Tracer</li>
    <li><b>Control de versiones:</b> Git & GitHub</li>
  </ul>


  <h2>🌱 Lo que estoy aprendiendo</h2>
  <ul>
    <li>Programación orientada a objetos (C#)</li>
    <li>Fundamentos de bases de datos</li>
    <li>Desarrollo de proyectos en equipo usando Git</li>
  </ul>

  <hr>

  <h2>📫 Contacto</h2>
  <ul>
    <li>✉ Correo: <a href="mailto:tu_correo@ejemplo.com">maveth.izquierdo92@cenyca.edu.mx</a></li>
  </ul>

  <hr>

  <p align="center">✨ Siempre abierto a aprender, colaborar y crear nuevos proyectos ✨</p>

  </p>


<!-- Connect with me -->
<!--h2 without bottom border-->
<div id="user-content-toc">
  <ul align="center">
    <summary><h2 style="display: inline-block">Conectate conmigo🤝</h2></summary>
  </ul>
</div>

<!--icons and links-->
<p align="center">
<a href="https://www.instagram.com/?next=%2Ffxcal%2Fdisclosure%2F%3Fnext%3D%252F" target="blank"><img align="center" src="https://user-images.githubusercontent.com/88904952/234981169-2dd1e58f-4b7e-468c-8213-034ba62156c3.png" alt="instagram" height="50" width="50" /></a>
<a href="https://1010nishant.hashnode.dev/" target="blank"><img align="center" src="https://user-images.githubusercontent.com/88904952/234982196-562aea17-5532-4550-8c08-1c7cb994a541.png" alt="hashnode" height="50" width="50" /></a>
  <a href="https://www.facebook.com/maveth.izquierdo/" target="blank">
    <img align="center" src="https://img.icons8.com/fluency/48/000000/facebook-new.png" 
         alt="facebook" height="50" width="50" />
  </a>


  
</p>




  <a target="_blank" align="center">
    <img align="right" top="500" height="300" width="400" alt="GIF"
      src="https://media.giphy.com/media/SWoSkN6DxTszqIKEqv/giphy.gif">
  </a>

</body>
</html>
